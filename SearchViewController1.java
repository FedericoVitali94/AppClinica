   ...
   private void handleSearchBut(ActionEvent event) {
      ...
      if (!selectedDisease.isEmpty()) {
         ...
         if (this.radioNow.isSelected()) {
            String firstBlock = "?pers cdata:SuffersFrom ?dis . \n" + diseaseFilter;
            whereClause = whereClause.concat(firstBlock);
         } else if (this.radioPast.isSelected()) {
            String firstBlock = "?pers cdata:SufferedFrom ?dis . \n" + diseaseFilter;
            whereClause = whereClause.concat(firstBlock);
         } else if (this.radioNowPast.isSelected()) {
            String firstUnion = "{\n" + whereClause;
            String secondUnion = whereClause;
            String firstBlock = "?pers cdata:SuffersFrom ?dis . \n" + diseaseFilter;
            firstUnion = firstUnion.concat(firstBlock + "} \n union \n { \n");

            String secondBlock = "?pers cdata:SufferedFrom ?dis . \n" + diseaseFilter + "} \n";
            secondUnion = secondUnion.concat(secondBlock);
            whereClause = firstUnion.concat(secondUnion);
         }
      }
      ...
      //execute the query
      this.mldg.setRulesets(SPARQLRuleset.OWL_HORST);
